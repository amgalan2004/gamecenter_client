  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 backdrop-blur-md bg-[#0f172a]/70 border-b border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between text-white">
        ...
        >
          <div className="p-2 bg-blue-600 rounded-xl group-hover:scale-110 group-active:scale-95 transition-all shadow-lg shadow-blue-600/30">
            <img
              src="/logo.svg"
              alt="Logo"
              className="w-5 h-5"
            ...
            />
            {/* Лого зураг байхгүй үед харагдах жижиг икон */}
            <div className="w-5 h-5 flex items-center justify-center">
                <svg viewBox="0 0 24 24" className="w-5 h-5 fill-white" xmlns="http://www.w3.org/2000/svg"><path d="M21 6H3c-1.1 0-2 .9-2 2v8c0 1.1.9 2 2 2h18c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm-10 7H8v3H6v-3H3v-2h3V8h2v3h3v2zm4.5 2c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zm4-3c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/></svg>
            </div>
          </div>
        ...
        {/* --- ТӨВ ХЭСЭГ: Үндсэн цэсүүд (Desktop) --- */}
        <div className="hidden md:flex items-center bg-white/5 p-1 rounded-full border border-white/10">
          ...
          {isAuthenticated && userRole === "PLAYER" && (
            <>
              <NavBtn active={location.pathname === "/gaming-center-map"} onClick={() => navigate("/gaming-center-map")}>
                Төвүүд
              </NavBtn>
              <NavBtn active={location.pathname === "/booking-history"} onClick={() => navigate("/booking-history")}>
                Захиалга
              </NavBtn>
              <NavBtn active={location.pathname === "/digital-wallet"} onClick={() => navigate("/digital-wallet")}>
                Түрийвч
              </NavBtn>
            </>
          )}
          ...
          {isAuthenticated && userRole === "CENTER_ADMIN" && (
            <>
              <NavBtn active={location.pathname === "/admin-dashboard"} onClick={() => navigate("/admin-dashboard")}>
                Хянах самбар
              </NavBtn>
              <NavBtn active={location.pathname === "/about"} onClick={() => navigate("/about")}>
                 Бидний тухай
              </NavBtn>
            </>
          )}
        ...
        <div className="flex items-center gap-3">
          {isAuthenticated ? (
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className="hidden sm:block px-4 py-2 text-gray-300 hover:text-white text-xs font-bold transition-all"
              >
                Нэвтрэх
              </button>
              <button
                onClick={() => navigate("/register")}
                className="px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition-all text-xs font-bold shadow-lg shadow-blue-600/20 active:scale-95"
              >
                Бүртгүүлэх
              </button>
            </div>
          ...
          {/* Mobile Menu Toggle */}
          <button
            className="md:hidden p-2 ml-1 hover:bg-white/5 rounded-lg transition-all"
      ...
      {/* --- MOBILE MENU --- */}
      {menuOpen && (
        <div className="md:hidden absolute top-16 left-0 w-full bg-[#0f172a] border-b border-white/5 px-6 py-8 flex flex-col gap-6 animate-in slide-in-from-top duration-300 shadow-2xl">
          ...
          <div className="pt-6 border-t border-white/5">
            {isAuthenticated ? (
              <button 
                onClick={handleLogout} 
                className="w-full py-4 bg-red-500/10 text-red-500 rounded-2xl font-bold text-sm border border-red-500/10"
              >
                Системээс гарах
              </button>
            ) : (
