...
const BookingTable = ({ 
  ...
  const itemsPerPage = 10; // Нэг хуудсанд харуулах мөрийн тоо
  ...
  // Төлөвийн икон тодорхойлох функц
  ...
  // Одоогийн хуудсанд харуулах өгөгдлийг шүүж авах
  const currentData = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    return bookings.slice(startIndex, startIndex + itemsPerPage);
  }, [bookings, currentPage]);

  // Хуудасны дугаар гаргах функц (Google Style)
        ...
        <table className="w-full border-collapse">
          <thead className="bg-muted/30 border-b border-border">
            <tr className="text-gray-400 text-[11px] uppercase tracking-widest">
              ...
              <th className="text-left p-4 font-semibold text-center">Seat</th>
              <th className="text-left p-4 font-semibold text-center">Duration</th>
              <th className="text-left p-4 font-semibold">Cost</th>
              <th className="text-left p-4 font-semibold text-center">Status</th>
          ...
          <tbody className="divide-y divide-border/50">
            {currentData.length > 0 ? (
                ...
                <tr key={booking?.id} className="hover:bg-muted/20 transition-all group">
                  ...
                  <td className="p-4 text-center">
                    <div className="flex items-center justify-center space-x-1">
                      ...
                      <span className="font-mono text-sm text-foreground">{booking?.seatNumber}</span>
                    </div>
                  </td>
                  <td className="p-4 text-center">
                    <div className="flex items-center justify-center space-x-1">
                      <Icon name="Timer" size={14} className="text-gray-500" />
                      <span className="text-sm text-foreground">{booking?.duration}</span>
                    </div>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-center">
                    <div className={`inline-flex items-center space-x-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase border ${getStatusColor(booking?.status)}`}>
              ...
              <tr>
                <td colSpan="8" className="p-20 text-center text-muted-foreground">Захиалга олдсонгүй</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <div className="p-6 border-t border-border flex flex-col items-center gap-4 bg-muted/10">
          <div className="flex items-center gap-4">
            <button
              ...
              className="text-primary disabled:text-gray-600 font-bold hover:underline text-sm uppercase tracking-widest transition-all"
            >
              Өмнөх
            </button>
